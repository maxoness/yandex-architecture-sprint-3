...
Схема сущностей для основных баз UserDatabase и DeviceControlDatabase

[ER Диаграмма UserDatabase и DeviceControlDatabase](diagrams/entities/ER_UserDatabase_DeviceControlDatabase.jpg)
...
Для запросов пользователей/специалистов от API Gateway и приоритетных синхронных вызовов между сервисами выбираю REST API 
- здесь не предполагается высоких RPS и больших объемов данных
- разработка проста и понятна  

Для передачи между сервисами основного потока команд на устройства, а также полученной телеметрии выбираю брокер сообщений Kafka
- снизить связанность между сервисами для независимого развертывания и повышения общей отказоустойчивости
- данных телеметрии может быть много и нагрузка от них может быть неравномерна в течение дня
- более гибкое масштабирование отдельных узлов системы

### 2. Документация API

#### OpenAPI для DeviceControlService (DeviceControlController):
[api-docs/DeviceControlController.yaml](./api-docs/DeviceControlController.yaml)

#### AsyncAPI для DeviceControlService (DeviceControlKafkaProducer/DeviceControlKafkaConsumer):
[api-docs/DeviceControlKafka.yaml](./api-docs/DeviceControlKafka.yaml)